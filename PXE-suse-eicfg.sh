...
function conf_ipxe(){
    ...
    then
        ...
        echo -e "${CYAN}Creating 'ei.cfg' file...${NC}"
        case "${parts[0]}" in Windows*)
            for win in Windows10 Windows11
            do
                touch $path/Installers/$win/sources/ei.cfg
                echo "[Channel]" > $path/Installers/$win/sources/ei.cfg
                echo "Retail" >> $path/Installers/$win/sources/ei.cfg
                echo "[VL]" >> $path/Installers/$win/sources/ei.cfg
                echo "0" >> $path/Installers/$win/sources/ei.cfg
            done
        esac
    fi
